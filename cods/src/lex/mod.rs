    ...
    pub fn lex(&mut self, string: &str) -> crate::Result<Vec<Token>> {
        ...
        while let Some(c) = lexer.next() {
            ...
            match c {
                ...
                '(' => self.new_atom(&mut lexer, Token::par(ParT::LRound, span))?,
                '[' => self.new_atom(&mut lexer, Token::par(ParT::LSquare, span))?,
                '{' => self.new_atom(&mut lexer, Token::par(ParT::LCurly, span))?,
                ')' => self.new_atom(&mut lexer, Token::par(ParT::RRound, span))?,
                ']' => self.new_atom(&mut lexer, Token::par(ParT::RSquare, span))?,
                '}' => self.new_atom(&mut lexer, Token::par(ParT::RCurly, span))?,
