...
pub fn lex(tokens: &[Token]) -> crate::Result<Vec<Item>> {
    ...
    for (i, p) in pars {
        if p.is_opening() {
            ...
            if let Some((start, end)) = matching_parenthesis(i, p, &mut par_stack)? {
...
fn matching_parenthesis(
    ...
    match par_stack.pop() {
        ...
        Some((_, open_par)) => {
            return Err(crate::Error::MismatchedParenthesis {
                opening: open_par,
                found: close_par,
            });
        }
        _ => return Err(crate::Error::UnexpectedParenthesis(close_par)),
    }
}
